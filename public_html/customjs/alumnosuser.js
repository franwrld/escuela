...
function eventListiners() {
    ...
    formAlumnosUser.addEventListener("submit",guardarAlumnosEdit);
...
function cargarDatos() {
    API.get(`alumnosuser/getAlumnosDeEscuela?id_school=${id_school}`)
      .then((data) => {
  ...
  function cargarEscuelasEdit(id) {
    return API.get(`alumnosuser/getOneEscuela?id=${id}`).then(data => {
        if (data.success) {
            ...
            data.records.forEach(escuela => {
                ...
                if (escuela.id_school == id) {
                   option.selected = true;
                }
                selectEscuela.appendChild(option);
            });
            // Si se desea que el usuario no cambie la escuela, deshabilitamos el select
...
function cargarGrados() {
    API.get("alumnosuser/getAllGrados").then(
        data => {
            if (data.success) {
...
function cargarSecciones() {
    API.get("alumnosuser/getAllSecciones").then(
        data => {
            if (data.success) {
            ...
            if ((index >= recordIni) && (index <= recordFin)) {
                html += `
                    <tr>
                        ...
                        <td>
                            <button type="button" class="buttoninfoalumnos" onclick="VerAlumnoInfoUser(${item.id_alumno})"><img src="public_html/iconos/alumnoprofile24px.png"></button>
...
function agregarAlumnos() {
    ...
    const idEscuelaSesion = "<?= $_SESSION['id_school'] ?>";
    document.querySelector("#id_school").value = idEscuelaSesion;
...
function editarAlumno(id) {
    ...
    API.get("alumnosuser/getOneAlumno?id=" + id)
      .then((data) => {
        if (data.success) {
          let record = data.records[0];
          // Cargamos los selects y cuando terminen, mostramos los datos
          Promise.all([
            cargarGrados(record.id_grado),
            cargarSecciones(record.id_seccion),
            cargarEscuelasEdit(record.id_school)
          ]).then(() => {
              mostrarDatosForm(record);
              const { latitud, longitud } = record;
...
function eliminarAlumno(id) {
    Swal.fire({
        ...
        if (resultado.isConfirmed) {
            API.get("alumnosuser/deleteAlumno?id=" + id)
    .then((data) => {
...
function VerAlumnoInfoUser(id_alumno) {
    API.get(`alumnosuser/getAlumnoInfoUser?id_alumno=${id_alumno}`)
        .then((data) => {
            if (data.success) {
